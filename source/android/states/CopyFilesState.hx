...
class CopyFilesState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		if (shouldCopy && copyLoop != null)
		{
			if (copyLoop.finished && canUpdate)
			{
				...
				{
					SUtil.applicationAlert(failedFiles.join('\n'), 'Failed To Copy ${failedFiles.length} File.');
					if (!FileSystem.exists('logs'))
						FileSystem.createDirectory('logs');
					File.saveContent('logs/' + Date.now().toString().replace(' ', '-').replace(':', "'") + '-CopyState' + '.logs', failedFiles.join('\n'));
				}
	...
	public function copyAsset()
	{
		...
		if (!FileSystem.exists(toFile))
		{
			...
			if (!FileSystem.exists(directory))
				#if android SUtil.mkDirs(directory); #end
			try
			{
